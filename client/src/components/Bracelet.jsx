...
import BraceletBrief from './BraceletBrief';
...
const Bracelet = () => {
  ...
  const filteredBracelets = BraceletData.filter((data) => {
  ...
  console.log('Filtered Bracelets:', filteredBracelets);
    ...
    <div className="flex">
      ...
      <div className='flex flex-wrap justify-evenly gap-5 ml-[310px] mt-8'>
        {filteredBracelets.length > 0 ? (
          filteredBracelets.map((data, index) => (
            <BraceletBrief key={index} data={data} />
          ))
        ) : (
