...
const BraceletBrief = ({ data }) => {
  ...
  return (
    <>
      <Link to={`/Bracelet/${data.id}`} className='p-4 w-72 h-80 mb-5 bg-white shadow-lg rounded-lg overflow-hidden transition-transform transform hover:scale-105'>
        <div className='relative w-full h-60'>
          <img src={data.image} alt="Bracelet" className='w-full h-full object-cover transition-opacity duration-300 hover:opacity-80' />
          <FaHeart 
